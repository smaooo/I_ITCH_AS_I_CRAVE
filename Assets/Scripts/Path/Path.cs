...
public class Path {
    ...
    {
        anchorPos = new Vector3(anchorPos.x, 0, anchorPos.z);
        points.Add(points[points.Count - 1] * 2 - points[points.Count - 2]);
    ...
    public void UpdatePath(Vector3 pos)
    {
        ...
        {
            points[i] = new Vector3(points[i].x, pos.y, points[i].z);

            if (i % 3 == 0)
            {
                points[i] = new Vector3(points[i].x - points[0].x + pos.x, points[i].y, points[i].z - points[0].z + pos.z);
